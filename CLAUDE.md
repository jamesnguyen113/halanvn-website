...
- Long-tail clusters (added 17/07/2026, Chinh Phú Thịnh competitor pattern):
  - `/xa-go/<size>/` — 16 pages (C60–C300, Z100–Z300) from `src/pages/xa-go/[size].astro` + `src/data/purlin.ts`. Barem tables are theoretical (±3%). **Numeric xà gồ prices are allowed but owner-supplied only**: fill `purlinPrices` and set `purlinPricesUpdated` (drives titles + sitemap lastmod); missing prices render "Liên hệ". Automation must never touch `purlin.ts`.
